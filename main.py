...
import io
import random
from flask import Flask, request, render_template, session, redirect,render_template_string, Response
...
cars.columns = ['name', 'subtitle', 'acceleration_in_sec', 'top_speed_km_per_h', 'range_km',
                'efficiency_wh_per_hour', 'fast_charge_speed_km_per_h', 'drive', 'number_of_seats', 'price_in_euros',
                'price_to_drop']
cars.drop(columns='price_to_drop', inplace=True)
numeric_columns = cars.loc[:, ['acceleration_in_sec', 'top_speed_km_per_h', 'range_km',
                               'efficiency_wh_per_hour', 'fast_charge_speed_km_per_h', 'number_of_seats',
                               'price_in_euros']]
...
def html_table():
    return render_template('index.html', tables=[cars_view.to_html(classes='data')], titles=cars_view.columns.values)

@app.route('/view', methods=("POST", "GET"))
def html_table1():
    return render_template('view.html',tables=[info.to_html(classes='data')],titles=cars_view.columns.values)
